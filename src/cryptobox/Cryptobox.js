...
class ReadOnlyStore extends Proteus.session.PreKeyStore {
  ...
  get_prekey(prekey_id) {
    return new Promise((resolve, reject) => {
      ...
      this.store.load_prekey(prekey_id).then((pk) => {
        resolve(pk);
      }).catch((e) => {
        reject(e);
      });
...
module.exports = class Cryptobox {
  constructor(store) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.store.load_identity().then((id) => {
        if (!id) {
          this.identity = Proteus.keys.IdentityKeyPair.new();
          this.store.save_identity(this.identity).then(() => {
            Object.freeze(this);
            resolve(this);
          });
        } else {
  ...
  session_from_prekey(client_id, pre_key_bundle) {
    return new Promise((resolve, reject) => {
      ...
      Proteus.session.Session.init_from_prekey(this.identity, bundle).then((session) => {
  ...
  session_from_message(session_id, envelope) {
    return new Promise((resolve, reject) => {
      ...
      Proteus.session.Session.init_from_message(this.identity, pk_store, env).then(([session, plaintext]) => {
  ...
  session_load(session_id) {
    return new Promise((resolve, reject) => {
      this.store.load_session(this.identity, session_id).then((session) => {
        if (!session) {
          resolve(null);
        }
